...
describe('Swagger Docs Decorator', () => {
    ...
    it('should apply specified swagger decorators to the correct properties in the controller class.', () => {
        ...
        jest.spyOn(SwaggerLib, 'ApiOperation').mockImplementation((o) => SpyableHoster.mDecor1);
        // @ts-ignore
        jest.spyOn(SwaggerLib, 'ApiOkResponse').mockImplementation((o) => SpyableHoster.mDecor2);
        // @ts-ignore
        jest.spyOn(SwaggerLib, 'ApiBadRequestResponse').mockImplementation((o) => SpyableHoster.mDecor3);
        jest.spyOn(SwaggerLib, 'ApiBody').mockImplementation((o) => SpyableHoster.mDecor4);
        jest.spyOn(SwaggerLib, 'ApiParam').mockImplementation((o) => SpyableHoster.mDecor5);

        // ===================

        class TestApiDocs {
            method1 = [ApiOperation({ summary: 'ApiOperation summary test' })];
            method2 = [ApiOperation({}), ApiOkResponse(), ApiBadRequestResponse(), ApiBody({})];
            method3 = [ApiOkResponse(), ApiParam({ name: 'test1' }), ApiParam({ name: 'test2' })];
        }
        ...
        expect(SpyableHoster.mDecor1).toBeCalledWith(TestController, 'method1', getDescriptor('method1'));
        expect(SpyableHoster.mDecor1).toBeCalledWith(TestController, 'method2', getDescriptor('method2'));

        expect(SpyableHoster.mDecor2).toBeCalledTimes(2);
        expect(SpyableHoster.mDecor2).toBeCalledWith(TestController, 'method2', getDescriptor('method2'));
        expect(SpyableHoster.mDecor2).toBeCalledWith(TestController, 'method3', getDescriptor('method3'));

        expect(SpyableHoster.mDecor3).toBeCalledTimes(1);
        expect(SpyableHoster.mDecor3).toBeCalledWith(TestController, 'method2', getDescriptor('method2'));

        expect(SpyableHoster.mDecor4).toBeCalledTimes(1);
        expect(SpyableHoster.mDecor4).toBeCalledWith(TestController, 'method2', getDescriptor('method2'));

        expect(SpyableHoster.mDecor5).toBeCalledTimes(2);
        expect(SpyableHoster.mDecor5).toBeCalledWith(TestController, 'method3', getDescriptor('method3'));
        expect(SpyableHoster.mDecor5).toBeCalledWith(TestController, 'method3', getDescriptor('method3'));
    ...
    it(`should apply decorators even when other properties' decorator lists have errors.`, () => {
        ...
        expect(SwaggerLib.ApiOperation).toBeCalledWith({ summary: 'ApiOperation summary test' });
